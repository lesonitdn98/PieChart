package tech.sonle.myapplication.custom.components;
...
public class YAxis extends AxisBase {
    ...
    public void setZeroLineWidth(float width) {
        this.mZeroLineWidth = Utils.Companion.convertDpToPixel(width);
    }
    ...
    public float getRequiredWidthSpace(Paint p) {
        ...
        float width = (float) Utils.Companion.calcTextWidth(p, label) + getXOffset() * 2f;

        float minWidth = getMinWidth();
        float maxWidth = getMaxWidth();

        if (minWidth > 0.f)
            minWidth = Utils.Companion.convertDpToPixel(minWidth);

        if (maxWidth > 0.f && maxWidth != Float.POSITIVE_INFINITY)
            maxWidth = Utils.Companion.convertDpToPixel(maxWidth);
    ...
    public float getRequiredHeightSpace(Paint p) {
        ...
        String label = getLongestLabel();
        return (float) Utils.Companion.calcTextHeight(p, label) + getYOffset() * 2f;
    }
