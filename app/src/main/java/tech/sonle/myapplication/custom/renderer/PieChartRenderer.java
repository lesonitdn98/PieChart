...
public class PieChartRenderer extends DataRenderer {
    ...
    public PieChartRenderer(PieChart chart, ChartAnimator animator,
        ...
        mCenterTextPaint.setTextSize(Utils.Companion.convertDpToPixel(12f));

        getMValuePaint().setTextSize(Utils.Companion.convertDpToPixel(13f));
        getMValuePaint().setColor(Color.WHITE);
        getMValuePaint().setTextAlign(Align.CENTER);
        ...
        mEntryLabelsPaint.setTextSize(Utils.Companion.convertDpToPixel(13f));
    ...
    public void drawData(Canvas c) {
        int width = (int) getMViewPortHandler().getChartWidth();
        int height = (int) getMViewPortHandler().getChartHeight();
    ...
    protected float getSliceSpace(IPieDataSet dataSet) {
        ...
        float spaceSizeRatio = dataSet.getSliceSpace() / getMViewPortHandler().getSmallestContentExtension();
    ...
    protected void drawDataSet(Canvas c, IPieDataSet dataSet) {
        ...
        float phaseX = getMAnimator().getPhaseX();
        float phaseY = getMAnimator().getPhaseY();
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            if ((Math.abs(dataSet.getEntryForIndex(j).getY()) > Utils.Companion.getFLOAT_EPSILON())) {
                visibleAngleCount++;
            }
        }
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            // draw only if the value is greater than zero
            if (!(Math.abs(e.getY()) > Utils.Companion.getFLOAT_EPSILON())) {
            ...
            if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= Utils.Companion.getFLOAT_EPSILON()) {
            ...
            if (drawInnerArc && (innerRadius > 0.f || accountForSliceSpacing)) {
                ...
                if (sweepAngleOuter % 360f > Utils.Companion.getFLOAT_EPSILON()) {
                    if (accountForSliceSpacing) {
            ...
            mBitmapCanvas.drawPath(mPathBuffer, getMRenderPaint());
    ...
    public void drawValues(Canvas c) {
        ...
        float offset = Utils.Companion.convertDpToPixel(5.f);
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            applyValueTextStyle((IDataSet) dataSet);

            float lineHeight = Utils.Companion.calcTextHeight(getMValuePaint(), "Q")
                    + Utils.Companion.convertDpToPixel(4f);
            ...
            iconsOffset.x = Utils.Companion.convertDpToPixel(iconsOffset.x);
            iconsOffset.y = Utils.Companion.convertDpToPixel(iconsOffset.y);
                ...
                if (mChart.needsHighlight(j)) {
                    if (drawXOutside || drawYOutside) {
                        ...
                        if (transformedAngle % 360.0 >= 90.0 && transformedAngle % 360.0 <= 270.0) {
                            ...
                            getMValuePaint().setTextAlign(Align.RIGHT);
                            ...
                            pt2x = pt1x + polyline2Width;
                            pt2y = pt1y;
                            getMValuePaint().setTextAlign(Align.LEFT);
                        ...
                        if (lineColor != ColorTemplate.COLOR_NONE) {
                            ...
                            // TODO draw outline value with line
...
//                        c.drawLine(pt1x, pt1y, pt2x, pt2y, mValueLinePaint);
    ...
    protected void drawHole(Canvas c) {
        ...
        if (mChart.isDrawHoleEnabled() && mBitmapCanvas != null) {
            ...
            if (Color.alpha(mTransparentCirclePaint.getColor()) > 0 &&
                ...
                mTransparentCirclePaint.setAlpha((int) ((float) alpha * getMAnimator().getPhaseX() * getMAnimator().getPhaseY()));
    ...
    public void drawHighlighted(Canvas c, List<? extends Highlight> indices) {
        ...
        for (Highlight highlight : indices) {
            ...
            for (int j = 0; j < entryCount; j++) {
                // draw only if the value is greater than zero
                if ((Math.abs(set.getEntryForIndex(j).getY()) > Utils.Companion.getFLOAT_EPSILON())) {
                    visibleAngleCount++;
                }
            }
            ...
            getMRenderPaint().setColor(mChart.getData().getEntryForHighlight(highlight).getColor());
